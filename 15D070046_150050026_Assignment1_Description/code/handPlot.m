...
h1 = figure;
hold on;
title('Initial pointsets - Hands')
for i = 1:N
    scatter(pointSets(1,:,i), pointSets(2,:,i), 5, rand(1,3)); 
end
hold off;
save('../results/Initial_Pointsets_Hands.mat','h1');
[z_mean, data_aligned] = mean_calc(data_preshaped, N, dim, num_points);
h2 = figure;
hold on;
title('Aligned point sets and mean shape - Hands');
for i = 1:N
...
legend(p1,'Mean');
hold off;
save('../results/AlignedPointsetsAndMeanHands.mat','h2');
[D_sorted, V_sorted]  = mode_variation(z_mean, data_aligned, N, dim, num_points);
h3 = figure;
hold on;
title('Plot of variances - Hands');
plot(diag(D_sorted));
hold off;
save('../results/PlotVarianceHands.mat','h3');
...
hold on;
title('Aligned point sets, mean shape and modes of variation- Hands');
for i = 1:N
    scatter(data_aligned(1,:,i), data_aligned(2,:,i), 5, rand(1,3));     
end
...
hold off;
save('../results/ModesOfVariationHands.mat','h4');
